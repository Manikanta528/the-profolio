...
function UsersProfile() {
  ...
  // to store the users projects data
  const [projects, setProjects] = useState([]);

  // to store the users details form the location state
  ...
  // to get the projects of the user
  useEffect(() => {
    if (location.state?.data.$id == undefined) {
      navigate("/Find");
    } else {
      const userProjects = databases.listDocuments(
        import.meta.env.VITE_DATABASE_ID,
        import.meta.env.VITE_USER_PROJECTS_COLLECTION_ID
      );
      userProjects.then(
        function (response) {
          const filteredProjects = response.documents.filter(
            (project) => project.userId == location.state?.data.$id
          );
          if (filteredProjects.length > 0) setProjects(filteredProjects);
          else setProjects([]);
        },
        function (error) {
          console.log(error);
  ...
  return (
    <div>
      <div className="bg-background dark:bg-backgroundDark text-textPrimary dark:text-textPrimaryDark px-6 md:px-16 max-h-fit min-h-screen">
        ...
        {projects.length > 0 && (
          <>
            <h1 className="font-bold text-xl md:text-2xl py-4 text-textPrimary dark:text-textPrimaryDark">
              {" "}
              Projects
            </h1>
            <div className="flex flex-wrap gap-8 pb-4">
              {projects.map((project, i) => (
                <ProjectCard key={i} data={project} findFlag={true} />
              ))}
            </div>
          </>
        )}
